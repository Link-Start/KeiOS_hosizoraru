...
internal sealed interface GitHubShareImportRestoreResult {
    data class Snapshot(val snapshot: GitHubShareImportStoredFlowSnapshot) :
        GitHubShareImportRestoreResult

    data class Coordinator(val result: ShareImportCoordinatorResult) :
        GitHubShareImportRestoreResult
}
...
internal class GitHubShareImportWindowCoordinator(
    ...
    suspend fun loadStoredSnapshot(): GitHubShareImportStoredFlowSnapshot {
        return withContext(ioDispatcher) {
            ...
            val preview = GitHubShareImportFlowStore
                .loadActivePreview()
                ?.toShareImportPreview()
            val managedInstallProgress = GitHubShareImportFlowStore
                .loadActiveManagedInstall()
                ?.toManagedInstallProgress()
            val attachCandidate = GitHubShareImportFlowStore
            ...
            GitHubShareImportStoredFlowSnapshot(
                ...
                attachCandidate = if (pendingTrack == null && preview == null) {
                    attachCandidate
                } else {
                    null
                }

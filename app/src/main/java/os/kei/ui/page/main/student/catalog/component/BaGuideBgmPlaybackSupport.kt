...
internal fun favoriteBgmCachedBytes(
    ...
    BaGuideTempMediaCache.cachedMediaBytes(
        context = context,
        sourceUrl = favoriteCacheScope(favorite),
...
internal fun favoriteBgmRuntimeState(
    ...
    val player =
        existingFavoriteBgmPlayer(context, favorite)
            ?: return BaGuideBgmPlaybackRuntimeState(volume = savedVolume)
...
internal fun prepareFavoriteBgmPlayback(
    ...
    val safeVolume = savedVolume.coerceIn(0f, 1f)
    if (player.volume != safeVolume) {
...
internal fun pauseFavoriteBgmPlayback(
    ...
    val runtimeState = favoriteBgmRuntimeState(context, favorite, savedVolume = savedVolume)
...
internal fun seekFavoriteBgmPlayback(
    ...
    val duration = player.duration.coerceAtLeast(0L)
    if (duration <= 0L) return favoriteBgmRuntimeState(context, favorite, savedVolume = savedVolume)
    val targetPosition =
        (duration * progress.coerceIn(0f, 1f))
            .toLong()
            .coerceIn(0L, duration)
    runCatching { player.seekTo(targetPosition) }
    return favoriteBgmRuntimeState(context, favorite, savedVolume = savedVolume)
}
...
internal fun updateFavoriteBgmVolume(
    ...
    return favoriteBgmRuntimeState(context, favorite, savedVolume = safeVolume)
}
...
private fun favoritePlaybackUrl(
    context: Context,
    favorite: GuideBgmFavoriteItem,
): String = resolveFavoriteBgmPlaybackUrl(context, favorite)

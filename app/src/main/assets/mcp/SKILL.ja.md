...
5. Read `{{RESOURCE_OVERVIEW_URI}}` or `keios://skill/tool/{tool}` for low-level detail.
...
3. GitHub source split: use `sourceMode=github_repository` or `sourceMode=direct_apk`
   on track list, export, check, and summary tools. `direct_apk` is the argument and storage id

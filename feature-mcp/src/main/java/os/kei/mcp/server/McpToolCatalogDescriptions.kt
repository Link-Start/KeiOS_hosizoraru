...
internal object McpToolCatalogDescriptions {
    val english = mapOf(
        ...
        "keios.os.cards.list" to "List OS Activity and shell cards. Args: target=activity|shell|all, query, onlyVisible, includeOutput, limit.",
        ...
        "keios.github.repo.package.scan" to "Repository -> package: scan a known repo's release APKs and read the package name, preferring expectedPackageName when provided. Use keios.github.package.repo.search for the opposite direction.",
        ...
        "keios.github.package.repo.search" to "Package -> repositories: find candidate repos for a known package name and verify their APKs. Use keios.github.repo.package.scan for the opposite direction. Args: packageName, appLabel, preferredRepoUrl, candidateLimit, verificationLimit.",
        ...
        "keios.ba.guide.cache.inspect" to "Inspect the Student Guide detail cache: one entry by URL, or the whole footprint when url is omitted. Args: url, includeSections, refreshIntervalHours.",
    ...
    private val chinese = mapOf(
        ...
        "keios.os.cards.list" to "列出 OS Activity 与 Shell 卡片。参数：target=activity|shell|all、query、onlyVisible、includeOutput、limit。",
        ...
        "keios.github.repo.package.scan" to "仓库 -> 包名：扫描已知仓库的最新稳定 release APK 并读取包名，传入 expectedPackageName 时优先匹配。反方向请用 keios.github.package.repo.search。",
        "keios.github.direct_apk.inspect" to "检查订阅项目或直链 APK manifest，输出包名、版本、SDK、ABI、权限与签名摘要。",
        "keios.github.package.repo.search" to "包名 -> 仓库：按已知包名查找候选仓库并验证其 APK 包名。反方向请用 keios.github.repo.package.scan。参数：packageName、appLabel、preferredRepoUrl。",
    ...
    private val japanese = mapOf(
        ...
        "keios.os.cards.list" to "OS Activity と Shell のカードを一覧します。引数: target=activity|shell|all、query、onlyVisible、includeOutput、limit。",
        ...
        "keios.github.repo.package.scan" to "リポジトリ -> package: 既知リポジトリの最新安定版 APK から package 名を読み取ります。expectedPackageName 指定時は優先します。逆方向は keios.github.package.repo.search。",
        "keios.github.direct_apk.inspect" to "購読プロジェクトまたは直接 APK URL の manifest を検査し、package、version、SDK、ABI、権限、署名概要を返します。",
        "keios.github.package.repo.search" to "package -> リポジトリ: 既知の package 名から候補リポジトリを探し、APK package を検証します。逆方向は keios.github.repo.package.scan。",

...
- Liquid Glass bottom sheet 使用更干净的档位链路：多数 sheet 默认 3/4 屏，内容溢出时自动扩展，
  顶部和底部 chrome 尽量保留内容可见区域，并补齐预测式返回的平滑关闭。
- GitHub 追踪继续打磨日常体验：失败过滤在恢复后自动回到全项目，Actions 新构建通知按项目独立存在，
  发行日志支持保留 Markdown 复制，更新提示文案更简洁。
- OS 工具新增更多内置活动入口，包括 HyperOS 全面屏设置、语言设置、应用内存用量、通知设置、应用管理、
  极暗、电池使用情况和跨设备服务。
- 活动 card 与 Shell card sheet 支持搜索和分类，用户卡片变多后维护更轻松。
- MCP 页面和内置 skill 补齐 Codex 辅助开发相关内容，密集工具组拆成更清晰的独立 card，BA 相关工具也独立呈现。
- 设置与导航细节覆盖短页面底栏恢复、点击 title card 唤回底栏、复制能力扩展的文字选择表现、发行日志长按选择和深色搜索可读性。
- 发布前收口 Apple Designs 图标在 benchmark 包的切换、R8 startup profile 警告、MMKV native 符号打包、
  Gradle 内存配置、Baseline Profile 刷新和相关链路测试。
...
- 当前稳定标签：[v1.7.0](https://github.com/hosizoraru/KeiOS/releases/tag/v1.7.0)。

...
private val GAMEKEE_DEFAULT_HEADERS =
    mapOf(
        "Accept" to "*/*",
...
private object GameKeeMediaPlayerCache {
    ...
    private fun analyzeResourceState(
        cache: SimpleCache,
        key: String,
    ): ResourceState {
    ...
    private fun refreshIntervalMs(): Long {
        val hours =
            runCatching { BASettingsStore.loadCalendarRefreshIntervalHours() }
    ...
    private fun mediaCacheDir(context: Context): File = File(context.cacheDir, GAMEKEE_MEDIA_CACHE_DIR)
...
internal fun shouldRunGameKeeMediaCacheCleanup(
    nowMs: Long,
    lastCleanupMs: Long,
    checkIntervalMs: Long,
): Boolean {
    if (checkIntervalMs <= 0L) return true
    return (nowMs - lastCleanupMs).coerceAtLeast(0L) >= checkIntervalMs
}

internal fun loadGameKeeMediaCacheDiagnostics(context: Context): GameKeeMediaCacheDiagnostics =
    GameKeeMediaPlayerCache.loadDiagnostics(context)

...
const val GITHUB_SHARE_IMPORT_ACTIVE_PREVIEW_MAX_AGE_MS = 25 * 60 * 1000L
const val GITHUB_SHARE_IMPORT_ACTIVE_RESULT_MAX_AGE_MS = 2 * 60 * 60 * 1000L

fun isGitHubShareImportPreviewExpired(createdAtMillis: Long, nowMillis: Long): Boolean {
    return createdAtMillis <= 0L ||
        (nowMillis - createdAtMillis).coerceAtLeast(0L) > GITHUB_SHARE_IMPORT_ACTIVE_PREVIEW_MAX_AGE_MS
}

fun isGitHubShareImportResultExpired(completedAtMillis: Long, nowMillis: Long): Boolean {

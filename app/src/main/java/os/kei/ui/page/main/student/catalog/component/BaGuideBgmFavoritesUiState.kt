...
internal enum class BaGuideBgmFavoriteSortMode(
...
internal enum class BaGuideBgmQueueMode(
    @param:StringRes val labelRes: Int,
) {
...
internal fun filterAndSortBgmFavorites(
    ...
    val filtered =
        if (query.isBlank()) {
            ...
            val searchContext = BgmFavoriteSearchContext.build()
            favorites.filter { item ->
                bgmFavoriteMatchesSearch(
                    favorite = item,
                    searchQuery = query,
    ...
    return when (sortMode) {
        BaGuideBgmFavoriteSortMode.Recent -> {
            filtered.sortedByDescending { it.favoritedAtMs }
        }

        BaGuideBgmFavoriteSortMode.AddedOrder -> {
            filtered.sortedBy { it.favoritedAtMs }
        }

        BaGuideBgmFavoriteSortMode.Student -> {
            filtered.sortedWith(
        ...
        BaGuideBgmFavoriteSortMode.Title -> {
            filtered.sortedWith(

    ...
    return when (kind) {
        BaGuideCatalogImportKind.Student -> {
            withContext(parseDispatcher) {
                ...
                BaGuideCatalogImportPreviewState(
                    ...
                    bgmPreview = bgmFavoriteRepository.previewImport(""),
                )
            }
        }
        ...
        BaGuideCatalogImportKind.Bgm -> {
            withContext(parseDispatcher) {
                BaGuideCatalogImportPreviewState(
                    ...
                    bgmPreview = bgmFavoriteRepository.previewImport(raw),
                )
            }
        }
        ...
        BaGuideCatalogImportKind.All -> {
            coroutineScope {
                ...
                val bgmPreview =
                    async(parseDispatcher) {
                        bgmFavoriteRepository.previewImport(raw)
                    }
                BaGuideCatalogImportPreviewState(
                    kind = kind,
                    raw = raw,
    ...
    coroutineScope {
        ...
        val bgmResult =
            when (preview.kind) {
                ...
                -> {
                    async(ioDispatcher) {
                        bgmFavoriteRepository.importMerged(preview.raw)
                    }
                }
...
internal suspend fun buildCatalogAllFavoritesExportJsonAsync(
    ...
    val bgmFavoritesJson =
        withContext(ioDispatcher) {
            bgmFavoriteRepository.buildExportJson()
        }
...
internal suspend fun writeBaGuideCatalogJsonExportToTreeAsync(
    ...
    return try {
        withContext(ioDispatcher) {
            ...
            val output =
                context.contentResolver.openOutputStream(targetDoc.uri)
                    ?: return@withContext false
            output.bufferedWriter().use { writer ->
                writer.writeTextCancellable(request.payload)
            }

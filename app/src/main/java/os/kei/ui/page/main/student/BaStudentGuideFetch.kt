...
private fun fetchGuideInfoByApi(
    sourceUrl: String,
    ...
    val contentId =
        extractGuideContentIdFromUrl(target)
            ?: error("unable to resolve content_id")

    val refererPath =
        runCatching { target.toUri().path.orEmpty() }
...
private fun buildGuideInfoFromContentDetail(
    ...
    return BaStudentGuideInfo(
        ...
        syncedAtMs = clock.nowMs(),
    )
}

private fun fetchGuideInfoFromHtml(
    sourceUrl: String,
    clock: BaGuideDataClock,
): BaStudentGuideInfo {
    val target = normalizeGuideUrl(sourceUrl)
...
internal fun fetchGuideInfo(
    sourceUrl: String,
    clock: BaGuideDataClock = BaGuideSystemDataClock,
): BaStudentGuideInfo {
    val apiResult = runCatching { fetchGuideInfoByApi(sourceUrl, clock) }
    apiResult.getOrNull()?.let { return it }

    val htmlResult = runCatching { fetchGuideInfoFromHtml(sourceUrl, clock) }
